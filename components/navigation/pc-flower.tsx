...
import { flower } from "@/components/navigation";
...
const Flower = () => {
  return (
    <div className="group delay-100 transition duration-300">
      <Link href={flower.href} className="hover:text-[#009a7b] hover:italic">
        {flower.label}
      </Link>
      <div className="group-hover:flex justify-center items-center gap-x-36 hidden w-full bg-[#f9f8f7] h-[40vh] absolute right-0 top-36">
        ...
        <div className="flex justify-between items-center">
          {flower.desc.map((item) => (
            <div className="flex flex-col" key={item.image}>
              <img
                src={item.image}
                className="w-[50%] h-[50%] object-fill mb-4"
              />
              <div className="flex items-center gap-x-12 hover:italic hover:text-[#009a7b] cursor-pointer">
