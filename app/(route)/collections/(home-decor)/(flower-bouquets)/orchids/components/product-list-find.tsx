...
const ProductFind: React.FC<ProductFind> = ({ items }) => {
  ...
  const handleChange = (value: string) => {
   let tempArray = items.filter(
     (product) => product.name.includes("Orchid")
   );

    if (value === "default") {
      setSortedProducts([...tempArray]);
    } else {
      tempArray = JSON.parse(JSON.stringify(tempArray)) as Product[];
      if (value === "high") {
        tempArray.sort(
          (product1, product2) =>
    ...
    <>
        ...
        <div className="py-6 border-b flex flex-col space-y-4">
          ...
          <div className="lg:flex items-center justify-between hidden">
            <span className=" text-neutral-500">
              The pet-friendly Phalaenopsis is one of the easiest varieties of
              orchids to grow as a houseplant.
            </span>
            ...
            >
              ...
              <div className="fixed inset-0 z-40 flex">
                <Dialog.Panel className="relative ml-auto flex h-full w-full max-w-xs flex-col overflow-y-auto bg-white py-4 pb-6 shadow-xl">
                  ...
                  <div className="p-4">
                    <div className="flex flex-col space-y-2 pb-3 border-b">
                      <span className="font-medium text-xl">More Final</span>
                      <span className="text-neutral-500 text-sm">
                        Browse collections
                      </span>
                    </div>
      ...
      <div className="space-y-4 mt-8 lg:flex block">
        <div className="lg:basis-1/5 mt-12 lg:flex hidden flex-col hover:cursor-pointer space-y-2 text-[1.1rem]">
          <Link href="/collections/orchids">Flower & Bouquets</Link>
          <span className="italic text-[#009a7b] font-medium flex items-center ">
            <p>Orchids</p>
            <Dot size={40} />
          </span>
          <Link href="/collections/blooming-plants">Flower Plants</Link>
          <Link href="/collections/dried-bouquets">Dried Bouquets</Link>
        </div>
        <div className="lg:basis-4/5">
          <div className="lg:grid-cols-4  grid grid-cols-1 md:grid-cols-2 gap-4 pb-8">
